...
def get_target_size(img_tensor):
    ...
    length = max(h, w)
    mod = length % minimum_size
    padded_length = min((mod + 1) * minimum_size, 640)
...
def pipeline_infernce(args, model, processor, example, bx, device):
    ...
    for i, cropped_tensor in enumerate(crops):
        ...
        target_size = get_target_size(cropped_tensor)
        layer_2_prediction_data = object_detection_inference(
            model=model,
            processor=processor,
            img_tensor=cropped_tensor,
            target_size=target_size,
            # thres=args.threshold,
            thres=0.0,
        )
...
def main(args, device=None):
    ...
    for example in tqdm(dataset, desc="Processing COCO dataset"):
        ...
        for it in range(args.it_num):
            bx, img_id, img_tensor, prediction_list = pipeline_infernce(args, model, processor, example, bx, device)
...
if __name__ == "__main__":
    ...
    NUM_EXAMPLES = 1000
    IT_NUM = 200
    SAVE_DIR = "./saved/attack/object_detection"
