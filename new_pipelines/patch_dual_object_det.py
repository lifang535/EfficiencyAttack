...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            ...
            dis_loss1 = calculate_distance_loss(raw1.pred_boxes * 640, [sh, sw, sh + ph, sw + pw])
            ...
            crops = []
            for r in res1:
                cs = crop(perturbed, r, device=device, pad_size=(320, 320))
                # cs = crop_interpolate_pad(perturbed, r, device,
                #                         interpolate_size=(320, 320))
                if len(cs) > 0: crops.extend(cs)
            num2 = 0
            if crops:
                ...
                cls_loss2 = calculate_cls_loss(raw2, args.target_2)
            else:
                cls_loss2 = torch.tensor(0.0, device=device, requires_grad=True)
            ...
            total = w1 * cls_loss1 + w2 * cls_loss2
            # total = dis_loss1
        

            torch.autograd.backward(total, retain_graph=True)
            ...
            log_dict[f"id_{image_id}"][f"iter_{i}"] = {
                ...
                "cls_loss1": cls_loss1.item(), "L2_loss": cls_loss2.item(), 
                "L_total": total.item()
            }
...
def main(args):
    ...
    model_1, proc_1 = load_from_pretrained(ckpt=args.ckpt_1, num_q=args.num_q_1, device=device)
    model_2, proc_2 = load_from_pretrained(ckpt=args.ckpt_2, num_q=args.num_q_2, device=device)
