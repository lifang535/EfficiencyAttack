...
class ObjectDetection(Process):
    ...
    def _infer(self, request):
        ...
        for i, result in enumerate(results): # lifang535 remove
            ...
            electric_appliance_list = ['microwave', 'oven', 'toaster', 'refrigerator', 'hair drier']
            car_number = sum([1 for label in result["labels"] if label in electric_appliance_list])
            ...
            for score, label, box in zip(result["scores"], result["labels"], result["boxes"]):
                ...
                if label == 'microwave' or label == 'oven' or label == 'toaster' \
                    or label == 'refrigerator' or label == 'hair drier': # lifang535 add: 电器
                    req = request.copy()
                    req.car_id = car_id
...
class VitDescription(Process):
    ...
    def run(self):
        print(f"[VitDescription] Start!")
        
        self.model = VisionEncoderDecoderModel.from_pretrained("nlpconnect/vit-gpt2-image-captioning")
        
        print(f"[VitDescription] loaded model 1")
        
        self.model.to(self.device)
        # self.model.device = self.device
        self.processor = ViTImageProcessor.from_pretrained("nlpconnect/vit-gpt2-image-captioning")
        
        print(f"[VitDescription] loaded model 2")
        
        self.tokenizer = AutoTokenizer.from_pretrained("gpt2")
        
        print(f"[VitDescription] loaded model 3")
        ...
        while not self.end_flag:
            ...
            # print(f"[VitDescription] video_id: {request.video_id}, frame_id: {request.frame_id}, car_id: {request.car_id}")
        ...
        if request.box is not None:
            ...
            with torch.no_grad():
                try: # add
                    ...
                    # print(f"[VitDescription] inputs: {inputs.shape}")
                    # print(f"[VitDescription] inputs: {inputs}")
                    # time.sleep(1000000)
                    inputs = self.processor(images=inputs, return_tensors="pt").to(self.device)
                    # print(f"[VitDescription] inputs: {inputs}")
                    # 使用模型生成图像描述
                    with torch.no_grad():
                        out = self.model.generate(**inputs, max_length=50, num_beams=4, early_stopping=True)

                    # 解码生成的文本
                    description = self.tokenizer.decode(out[0], skip_special_tokens=True)
                    # description = ""
                    
                except Exception as e:
                    print(f"[VitDescription] Error: {e}")
                    print(f"[VitDescription] inputs: {inputs}")
                    description = ""
            ...
            #     print(f"[VitDescription] label: {label}") # TODO: TEST 应该打印不出来，因为没有电器

            request.label += f": {label}"
            print(f"[VitDescription] label: {request.label}")
            
            # print(f"[VitDescription] video_id: {request.video_id}, frame_id: {request.frame_id}, car_id: {request.car_id}, label: {request.label}")
...
class FrameToVideo(Process):
    ...
    def check_frame(self, video_id, frame_id):
        ...
        if len(self.video_dict[video_id][frame_id]['car']) == car_number:
            ...
            self.times['pr'].append(self.video_dict[video_id][frame_id]['times']['pr'])
