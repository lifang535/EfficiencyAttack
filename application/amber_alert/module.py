...
class AgeRecognition(Process):
    ...
    def run(self):
        ...
        print(f"[AgeRecognition] loaded model 1")
        self.model = torch.load(self.model_path, map_location=self.device)
        self.id2label = self.model.config.id2label
        self.model.to(self.device)
        print(f"[AgeRecognition] loaded model 2")
        ...
        while not self.end_flag:
            ...
            # print(f"[AgeRecognition] video_id: {request.video_id}, frame_id: {request.frame_id}, car_id: {request.car_id}")
    ...
    def _infer(self, request):
        ...
        if request.box is not None:
            ...
            with torch.no_grad():
                try: # add
                    inputs = self.image_processor(images=[inputs], return_tensors="pt").to(self.device)
                    ...
                    print(f"[AgeRecognition] Error: {e}")
                    print(f"[AgeRecognition] inputs: {inputs}")
                    label = ""
            ...
            #     print(f"[AgeRecognition] label: {label}") # TODO: TEST 应该打印不出来，因为没有电器

            request.label += f": {label}"
            print(f"[AgeRecognition] label: {request.label}")
            
            # print(f"[AgeRecognition] video_id: {request.video_id}, frame_id: {request.frame_id}, car_id: {request.car_id}, label: {request.label}")
...
class PoseRecognition(Process):
    ...
    def run(self):
        print(f"[PoseRecognition] Start!")
        self.model = torch.load(self.model_path, map_location=self.device)
        
        print(f"[PoseRecognition] loaded model 1")
        ...
        while not self.end_flag:
            ...
            # print(f"[PoseRecognition] video_id: {request.video_id}, frame_id: {request.frame_id}, car_id: {request.car_id}")
        ...
        if request.box is not None:
            ...
            with torch.no_grad():
                try: # add
                    ...
                    print(f"[PoseRecognition] Error: {e}")
                    print(f"[PoseRecognition] inputs: {inputs.shape}")
                    candidate, subset = [], []
            ...
            #     sorted_results = sorted(results, key=lambda x: x[2], reverse=True)
            #     label = sorted_results[0][1]
            #     print(f"[PoseRecognition] label: {label}") # TODO: TEST 应该打印不出来，因为没有电器

            request.label += f": {label}"
            print(f"[PoseRecognition] label: {request.label}")
            
            # print(f"[PoseRecognition] video_id: {request.video_id}, frame_id: {request.frame_id}, car_id: {request.car_id}, label: {request.label}")
...
class FrameToVideo(Process):
    ...
    def car_get(self):
        while not self.end_flag:
            ...
            if frame_id not in self.video_dict[video_id]:
                ...
                self.video_dict[video_id][frame_id]['times'] = {'od': request.times[0] - request.start_time, \
                    'face_recognition': request.times[1] - request.start_time, \
                    'age_recognition': request.times[2] - request.start_time, \
                    'pose_recognition': request.times[3] - request.start_time}
                self.video_dict[video_id][frame_id]['flops'] = {'od': request.flops[0], \
                    'face_recognition': request.flops[1], \
                    'age_recognition': request.flops[2], \
                    'pose_recognition': request.flops[3]}
            
            self.video_dict[video_id][frame_id]['times']['face_recognition'] = request.times[1] - request.start_time
            self.video_dict[video_id][frame_id]['flops']['face_recognition'] += request.flops[1]
            
            self.video_dict[video_id][frame_id]['times']['age_recognition'] = request.times[2] - request.start_time
            self.video_dict[video_id][frame_id]['flops']['age_recognition'] += request.flops[1]
            
            self.video_dict[video_id][frame_id]['times']['pose_recognition'] = request.times[3] - request.start_time
            self.video_dict[video_id][frame_id]['flops']['pose_recognition'] += request.flops[1]
    ...
    def check_frame(self, video_id, frame_id):
        ...
        if len(self.video_dict[video_id][frame_id]['car']) == car_number:
            ...
            self.times['face_recognition'].append(self.video_dict[video_id][frame_id]['times']['face_recognition'])
            self.times['age_recognition'].append(self.video_dict[video_id][frame_id]['times']['age_recognition'])
            self.times['pose_recognition'].append(self.video_dict[video_id][frame_id]['times']['pose_recognition'])
            self.times['llm'].append(time.time() - request.start_time)
