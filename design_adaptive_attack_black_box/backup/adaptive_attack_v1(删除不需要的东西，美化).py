...
sum_iter = 200 # lifang535: for adaptive attack
# def run_attack(outputs,bx, strategy, max_tracker_num, adam_opt):
#     global epoch_id, sum_iter
...
#     loss_bx = torch.norm(bx, p=2)
#     loss_bx = loss_bx / 50.0
    
#     # loss_bx *= 255.0 # lifang535: 乘以 255.0，使得 loss_bx 和 loss_score 在同一个数量级
#     # loss_bx /= 8.0 # eps = 8.0
    
#     # # 归一化 loss
#     # import math
#     # loss_score = loss_score / math.sqrt(loss_score.numel())
#     # loss_area = loss_area / math.sqrt(loss_area.numel())
#     # loss_bx = loss_bx / math.sqrt(loss_bx.numel())
    
#     # lifang535: 静态损失函数
#     # loss = 10*loss_score+loss_area+10*loss_bx # lifang535 delete
#     # # lifang535: 动态损失函数
#     # if epoch_id <= 50:
#     #     loss = loss_score
#     # else:
...
#     # loss = loss_score+loss_area+loss_bx
...
#     alpha2 = 1 - cos(min(epoch_id / sum_iter * pi, pi / 2))
#     alpha3 = 1 - cos(min(epoch_id / sum_iter * pi, pi / 2))
...
def run_attack(outputs_list, bx, strategy, max_tracker_num, adam_opt):
    global epoch_id, sum_iter
...
class AdaptiveAttack:
    ...
    def evaluate(
        ...
        for iter in tqdm(range(epochs)):
            ...
            output_1 = model_1(noise_imgs)
            output_2 = model_2(noise_imgs)
            output_3 = model_3(noise_imgs)
            
            outputs_list = [output_1, output_2, output_3]
            # outputs_list = [output_1]
            
            # bx = run_attack(outputs,bx, strategy, max_tracker_num, adam_opt)
            bx = run_attack(outputs_list,bx, strategy, max_tracker_num, adam_opt) # substitute outputs with output_1, output_2, output_3
...
def infer_yolov8(image_path, model):
    ...
    # outputs = non_max_suppression(outputs, conf_thres, iou_thres, max_det=max_det)
    # outputs = non_max_suppression(outputs_copy, conf_thres, iou_thres, max_det=max_det)
...
if __name__ == "__main__":
    ...
    weights_n = "../model/yolov5/yolov5n.pt" # yolov5n.pt yolov5s.pt yolov5m.pt yolov5l.pt yolov5x.pt
    weights_s = "../model/yolov5/yolov5s.pt" # yolov5n.pt yolov5s.pt yolov5m.pt yolov5l.pt yolov5x.pt
    weights_m = "../model/yolov5/yolov5m.pt" # yolov5n.pt yolov5s.pt yolov5m.pt yolov5l.pt yolov5x.pt
    weights_l = "../model/yolov5/yolov5l.pt" # yolov5n.pt yolov5s.pt yolov5m.pt yolov5l.pt yolov5x.pt
