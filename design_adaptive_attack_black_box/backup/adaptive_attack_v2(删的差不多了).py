...
def run_attack(outputs_list, bx, adam_opt):
    global epoch_id, sum_iter
...
class AdaptiveAttack:
    ...
    def evaluate(
        ...
        for iter in tqdm(range(epochs)):
            ...
            if image_process_func is None:
                ...
                processed_imgs = add_gaussian_noise(added_imgs) # 为图片添加高斯噪声
            elif image_process_func == "spatial_smoothing":
                processed_imgs = add_spatial_smoothing(added_imgs) # 为图片添加空间平滑
            else:
    ...
    def run(self):
        ...
        for image, image_name in zip(self.image_list, self.image_name_list):
            ...
            l1_norm, l2_norm = self.evaluate(image, image_name)
