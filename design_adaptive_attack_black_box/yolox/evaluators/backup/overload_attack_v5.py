...
def generate_mask(detection_results, x_shape, y_shape):
    ...
    # print(detection_results.shape) # lifang535 remove
    ...
    boxes = detection_results["boxes"] # lifang535
...
def run_attack(outputs,bx, strategy, max_tracker_num, mask):
    ...
    scores = outputs["scores"] # lifang535
    ...
    print(f"bx.grad = {bx.grad}") # lifang535
    time.sleep(20)
    ...
    def evaluate(
        ...
        def process_img(imgs, path):
            ...
            for iter in tqdm(range(epochs)):
                ...
                if iter == 0:
                    mask = generate_mask(result,added_imgs.shape[3],added_imgs.shape[2]).to(device) # lifng535: 掩码只改变一次
                bx = run_attack(result,bx, strategy, max_tracker_num, mask)
