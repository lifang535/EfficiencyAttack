...
def worker(gpu, args):
    ...
    for example in tqdm(local_dataset, desc=f"GPU {gpu}", position=gpu):
        ...
        for i, res in enumerate(results):
            cropped_list = crop_interpolate_pad(img_tensor, res, device, interpolate_size=(320, 320))
            if len(cropped_list) == 0:
                continue
            cat_cropped_tensor = torch.cat(cropped_list, dim=0)
            cat_target_size = get_target_size(cat_cropped_tensor)
        ...
        log_dict[f"id_{image_id}"]["L1_num"] = int(num_dets_1)
        log_dict[f"id_{image_id}"]["L2_num"] = int(num_dets_2)
