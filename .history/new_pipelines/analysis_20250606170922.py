...
def clean():
    ...
    for rank_0_file in tqdm(rank_0_files):
        ...
        print(f"File: {cmm}")
        print(f"Layer 1 Mean: {mean_1:3.4f}, Std: {std_1:3.4f}")
        print(f"Layer 2 Mean: {mean_2:3.4f}, Std: {std_2:3.4f}")
        print("Layer 1 distribution:")
        print(f"1 sigma: {mean_1 - std_1:3.4f}, 2 sigma: {mean_1 + std_1:3.4f}")
        print(f"2 sigma: {mean_1 - 2*std_1:3.4f}, 2 sigma: {mean_1 + 2*std_1:3.4f}")
        print(f"3 sigma: {mean_1 - 3*std_1:3.4f}, 3 sigma: {mean_1 + 3*std_1:3.4f}")
        print("Layer 2 distribution:")
        print(f"1 sigma: {mean_2 - std_2:3.4f}, 2 sigma: {mean_2 + std_2:3.4f}")
        print(f"2 sigma: {mean_2 - 2*std_2:3.4f}, 2 sigma: {mean_2 + 2*std_2:3.4f}")
        print(f"3 sigma: {mean_2 - 3*std_2:3.4f}, 3 sigma: {mean_2 + 3*std_2:3.4f}")
        print("\n" + "="*50 + "\n")
