...
def main(args):
    ...
    # engine_1 = init_inference(
    #     model_1,
    #     mp_size=1,
    #     dtype=torch.float32,
    #     replace_with_kernel_inject=False
    # )
    # engine_2 = init_inference(
    #     model_2,
    #     mp_size=1,
    #     dtype=torch.float32,
    #     replace_with_kernel_inject=False
    ...
    engine_1, _, _, _ = deepspeed.initialize(
        model=model_1,
        config=ds_config
    )
    engine_2, _, _, _ = deepspeed.initialize(
        model=model_2, 
        config=ds_config
    )
    ...
    for example in tqdm(coco_val_set):
        ...
        for i in tqdm(range(args.num_it)):
            ...
            for _, res in enumerate(results):
                ...
                cat_raw = engine_2(cat_cropped_tensor)
