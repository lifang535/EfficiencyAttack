...
def main(args):
    ...
    for example in tqdm(coco_val_set):
        ...
        # results = object_detection_inference(model_1, processor_1, bx_img_tensor, target_size, args.thres_1, device=device) # type(results) is list
        results = model_1(bx_img_tensor)
        num_dets_1 = get_num_dets(results, processor_1, target_size, args.thres_1)
        ...
        for i, res in enumerate(results):
            ...
            cat_results = model_2(cat_cropped_tensor)
            num_dets_2 += get_num_dets(cat_results, processor_2, cat_target_size, args.thres_2)
            pdb.set_trace()
...
def worker(gpu, args):
    ...
    out_path = os.path.join(
        args.output_dir,
        f"RANK_{gpu}_NUM_Q_1_{args.num_q_1}_THRES_1_{args.thres_1}"
