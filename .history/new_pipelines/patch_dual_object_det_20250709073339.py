...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            ...
            mask_bbox = torch.tensor([sw, sh, sw+ pw, sh + ph], device=device, dtype=torch.float32)
            ...
            distance_loss1 = calculate_distance_loss(res1[0], mask_bbox)
            iou_loss1 = calculate_iou_loss(res1[0], mask_bbox)
            ...
            log_dict[f"id_{image_id}"][f"iter_{i}"] = {
                ...
                "cls_loss1": cls_loss1.item(), "distance_loss1": distance_loss1.item(), "iou_loss1": iou_loss1.item(), "L2_loss": loss2.item().item(), 
