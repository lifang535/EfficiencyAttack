    ...
    for rank_file in tqdm(files):
        ...
        for k, v in data.items():
            for i in range(200):
                ...
                layer_1_loss.append(
                    v.get(f"iter_{i}").get("L1_loss", 0)
                )
                
                layer_2_loss.append(
                    v.get(f"iter_{i}").get("L2_loss", 0)
                )
                
                layer_total_loss.append(
                    v.get(f"iter_{i}").get("L_total", 0)
                )

    pdb.set_trace()
    mean_layer_1_det = np.mean(layer_1_num_det)
    mean_layer_2_det = np.mean(layer_2_num_det)
    mean_layer_1_loss = np.mean(layer_1_loss)
    mean_layer_2_loss = np.mean(layer_2_loss)
    mean_layer_total_loss = np.mean(layer_total_loss)
