...
def worker(gpu, args):
    ...
    for example in tqdm(local_dataset, desc=f"GPU {gpu}", position=gpu):
        ...
        for i in range(args.num_it):
            ...
            log_dict[f"id_{image_id}"][f"layer1_num_detections"] = num_dets_1
            log_dict[f"id_{image_id}"][f"layer2_num_detections"] = num_dets_2
            log_dict[f"id_{image_id}"][f"L1_loss"] = final_loss1
            log_dict[f"id_{image_id}"][f"L2_loss"] = final_loss2
            log_dict[f"id_{image_id}"][f"L_total"] = final_loss1 + final_loss2 if final_loss1 is not None and final_loss2 is not None else None

    output_path = os.path.join(args.output_dir,
        f"RANK_{gpu}_NUM_Q_1_{args.num_q_1}_THRES_1_{args.thres_1}_NUM_Q_2_{args.num_q_2}_THRES_2_{args.thres_2}.json")
    with open(output_path, "w") as f:
        json.dump(log_dict, f, indent=4)
...
if __name__ == "__main__":
    ...
    TARGET_2 = 2
    OUTPUT_DIR = "./saved/attacked/detr"
