...
def calculate_distance_loss(pred_boxes, patch_bbox):
    
    det_boxes = pred_boxes[0]
    if det_boxes is None or len(det_boxes) == 0:
        return torch.tensor(0.0, device=patch_bbox.device, requires_grad=True)
    ...
    return (left_diff + right_diff + top_diff + bottom_diff).mean()
