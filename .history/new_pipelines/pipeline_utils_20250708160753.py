...
def calculate_iou_loss(res, patch_shape):
    px1, py1, px2, py2 = patch_shape
    iou_loss = 0.0
    cnt = 1
...
def calculate_distance_loss(res, patch_shape):
    px1, py1, px2, py2 = patch_shape
    distance_loss = 0.0
    cnt = 1
    for box in res.get("boxes", []):
        ...
        if x2 > x1 and y2 > y1:
            left_margin = max(0, px1 - x1)
            right_margin = max(0, x2 - px2)
            top_margin = max(0, py1 - y1)
            bottom_margin = max(0, y2 - py2)
            distance = (left_margin + right_margin + top_margin + bottom_margin) / 4
    return distance_loss / cnt
