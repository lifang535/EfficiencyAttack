...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            ...
            hs = raw1.decoder_hidden_states
            decoder_feat = hs[-1]
            bbox_head = model_1.bbox_embed[-1]
            bbox_pred = bbox_head(decoder_feat) 
            ...
            test_output = raw1.pred_boxes.sum()

            # Use torch.autograd.grad to directly compute the gradient of the output
            # with respect to the input `bx`.
            (test_grad,) = torch.autograd.grad(outputs=test_output, inputs=bx, retain_graph=True)

            # Check if the resulting gradient tensor is all zeros.
            if torch.all(test_grad == 0):
                print("---")
                print("DEBUG CONFIRMED: The gradient of box predictions w.r.t. the input is ZERO.")
                print("The problem is inside the model's forward pass. It likely contains a non-differentiable")
                print("operation (like round, argmax, or integer casting) in the path to the box prediction head.")
                print("---")
            else:
                print("---")
                print("DEBUG: The gradient is NON-ZERO. This is a very unusual situation.")
                print(f"Gradient norm: {torch.norm(test_grad)}")
                print("---")
