...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            # prepare mask
            if args.use_patch and args.patch_placement == "center":
            ...
            probs1 = F.sigmoid(raw1.logits)
            cls_loss_target = get_cls_loss_target(probs1, args.target_1)
            cls_loss = F.mse_loss(probs1, cls_loss_target, reduction='sum') / (probs1.shape[1] + 1)
            loss1 = cls_loss
            res1 = processor_1.post_process_object_detection(raw1,
                        target_sizes=target_size, threshold=args.thres_1)
            ...
            if crops:
                ...
                loss2 = object_detection_loss(raw2, args.target_2)
            ...
            log_dict[f"id_{image_id}"][f"iter_{i}"] = {
                ...
                "L1_loss": loss1.item(), "L2_loss": loss2.item(),
                "L_total": (loss1 + loss2).item()
            }
    return log_dict
