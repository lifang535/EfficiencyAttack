...
def main(args):
    ...
    model_1 = deepspeed.init_inference(
        model_1,
        mp_size=1,
        dtype=torch.float16,
        replace_method='auto',
    ...
    model_2 = deepspeed.init_inference(
        model_2,
        mp_size=1,
        dtype=torch.float16,
        replace_method='auto',
    ...
    if dist.is_initialized():
        sampler = DistributedSampler(coco_val_set, shuffle=False)
        dataloader = DataLoader(coco_val_set, batch_size=1, sampler=sampler)
    else:
        dataloader = DataLoader(coco_val_set, batch_size=1, shuffle=False)
    ...
    for example in tqdm(dataloader):
        ...
        for i in tqdm(range(args.num_it)):
            ...
            for _, res in enumerate(results):
                cropped_list = crop_interpolate_pad(bx_img_tensor, res, device, interpolate_size=(128, 128))
                # cropped_list.append()
