...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            ...
            decoder_feat = raw1.last_hidden_state.detach().clone().requires_grad_()
            bbox_raw = model_1.bbox_embed[-1](decoder_feat)
            bbox_pred = torch.sigmoid(bbox_raw)

            # dummy loss to check
            loss = bbox_pred.mean()
            loss.backward()
            print("decoder_feat.grad.abs().sum():", decoder_feat.grad.abs().sum())
