...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            ...
            decoder_feat = raw1.last_hidden_state            # (1, 50, 256)
            bbox_raw = model_1.bbox_embed[-1](decoder_feat)  # (1, 50, 4)
            print("bbox_raw:", bbox_raw.mean(), bbox_raw.min(), bbox_raw.max())

            bbox_pred = torch.sigmoid(bbox_raw)
            print("bbox_pred:", bbox_pred.mean(), bbox_pred.min(), bbox_pred.max())
            ...
            distance_loss1 = calculate_distance_loss(bbox_pred, patch_bbox)
            iou_loss1 = calculate_iou_loss(bbox_pred, patch_bbox)
            ...
            total = cls_loss1 + distance_loss1 + iou_loss1 
