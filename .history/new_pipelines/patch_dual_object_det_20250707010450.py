...
def main(args):
    ...
    for example in tqdm(coco_val_set):
        ...
        if args.use_patch and args.patch_size <= min(width, height):
        ...
        for i in range(args.num_it):
            ...
            if args.patch_placement == "random" and args.use_patch:
            ...
            elif args.patch_placement == "center" and args.use_patch:
                img_h, img_w = img_tensor.shape[2], img_tensor.shape[3]
                patch_h, patch_w = bx.shape[2], bx.shape[3]
...
def worker(gpu, args):
    ...
    for example in tqdm(local_dataset, desc=f"GPU {gpu}", position=gpu):
        ...
        #     mask[:, :, :args.patch_size, :args.patch_size] = 1.0
        #     bx = torch.zeros_like(img_tensor).to(device).requires_grad_(True)
        # else:
        ...
        for i in range(args.num_it):
            ...
            if args.patch_placement == "center" and args.use_patch:
                ...
                perturbed_img = img_tensor * (1 - mask) + bx * mask
            else:
                # mask = torch.ones_like(img_tensor)
                perturbed_img = img_tensor + bx
            # ### MODIFICATION END ###
