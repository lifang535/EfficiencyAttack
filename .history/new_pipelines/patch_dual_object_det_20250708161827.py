...
def run_attack(dataset, model_1, processor_1, model_2, processor_2, args, device):
    ...
    for example in tqdm(dataset):
        ...
        for i in range(args.num_it):
            ...
            elif args.use_patch and args.patch_placement == "random":
                raise ValueError("Random patch placement is not supported in this attack.")
            elif args.use_patch == False:
            ...
            distance_loss1 = calculate_distance_loss(raw1, [sw, sh, sw+ pw, sh + ph])
            iou_loss1 = calculate_iou_loss(raw1, [sw, sh, sw+ pw, sh + ph])
            

            loss1 = cls_loss1 + distance_loss1 + iou_loss1
