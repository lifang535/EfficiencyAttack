    ...
    for example in tqdm(coco_val_set):
        ...
        for i in tqdm(range(args.num_it)):
            ...
            for _, res in enumerate(results):
                ...
                patch_size = 8
                for j in range(0, len(cropped_list), patch_size):
                    start = j
                    if start >= len(cropped_list):
                        break
                    end = start + patch_size
                    if end > len(cropped_list):
                        end = len(cropped_list)
                        
                    cat_cropped_tensor = torch.cat(cropped_list[ start : end ], dim=0)
            ...
            loss = loss_1 + loss_2
            if bx.grad is not None:
                bx.grad.zero_()
            loss.backward()  
            print(loss)
            with torch.no_grad():
                grad_norm = torch.norm(bx.grad, p=2) + 1e-20
                bx.data = bx.data - 1.5 * (bx.grad / grad_norm)  
                bx.data.clamp_(-0.04, 0.04)
            del loss, loss_1, loss_2, raw, results, cat_raw, cat_results
